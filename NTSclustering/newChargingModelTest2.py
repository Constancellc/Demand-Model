...
with open(trip_data,'rU') as csvfile:
    ...
    for row in reader:
        ...
        journeyLogs[vehicle].append([day,start,end,kWh,row[-1]])
...
with open(charge_data,'rU') as csvfile:
    ...
    for row in reader:
        ...
        chargeLogs[vehicle].append([day,start,end])
...
# ok, I think I've been a fucking idiot, because I know the vehicles SOC?
'''   
mc_runs = []
# now need to generate the pdf for the single week, let's start with once
for v in journeyLogs:
    charge = [0]*48*dayMax
    for mc in range(10000):
        jLog = sorted(journeyLogs[v])
        jLog.append([dayMax,48,0,0,])
        d = jLog[0][0] # day
        t = 0 # time
        j = 0
        SOC = 0.99
        ...
        # ok, what I want is to step in time, check if there has been a journey
        # (note that there may now be two... could want to fix that) etc.
        while d <= dayMax:
            if t == jLog[j][2]:
                ...
                start = chargeAfterJourney(dt,k,int(SOC*6),t)
                if start is True:
                    charge[48*(d-1)+t] += 1
                    SOC = 0.99
                j += 1
                ...
                try:
                    ...
                    dt = '0'
                start = randomCharge(dt,int(SOC*6),t)
                if start is True:
                    charge[48*(d-1)+t] += 1
                    SOC = 0.99
                [t,d] = step(t,d)
...
with open('res.csv','w') as csvfile:
    ...
    for t in range(48*(dayMax-7),48*dayMax):
        writer.writerow([charge[t]])
