...
plt.figure(figsize=(4.5,6.2))
plt.rcParams["font.family"] = 'serif'
plt.rcParams['font.size'] = 12
plt.subplot(2,1,1)
plt.title('100%',fontweight='bold')
plt.plot(av['wt'],c='k',ls=':',label='No Charging')


u2 = []
for t in range(288):
    u2.append(u['wt'][t]+av['wt'][t])
    #d[tm[f]][t] += av[tm[f]][t]
    
    #plt.title(ttls[f])
    #plt.plot(d[tm[f]],label='Uncontrolled (a)',c='g',ls='--')
plt.plot(u2,label='Uncontrolled',c='b')
plt.plot(fill(av['wt'],sum(u['wt'])),label='Controlled',c='r',ls='--')
plt.ylabel('Power (GW)')
...
u3 = []
for t in range(288):
    u3.append(av['wt'][t]+0.694*u['wt'][t])

plt.plot(u3,label='Uncontrolled',c='b')
plt.plot(fill(av['wt'],0.694*sum(u['wt'])),label='Controlled',c='r',ls='--')
plt.ylabel('Power (GW)')
plt.xticks(np.linspace(23,263,num=5),['02:00','07:00','12:00','17:00','22:00'])
plt.ylim(20,69)
plt.grid()
...
plt.savefig('../../Dropbox/papers/Nature/img/jan_national.eps', format='eps',
