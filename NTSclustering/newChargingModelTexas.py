...
trip_data = '../../Documents/NHTS/constance/texas-trips.csv'
...
NTS = {}
with open(stem+'texasLabels.csv','rU') as csvfile:
    reader = csv.reader(csvfile)
    for row in reader:
        try:
            NTS[row[0]] = int(row[1])
            ...
            continue
with open(stem+'texasLabelsWE.csv','rU') as csvfile:
    reader = csv.reader(csvfile)
    for row in reader:
        try:
            NTS[row[0]] = int(row[1])
...
# function: predictCharging
    def predictCharging(self,vehicle,power=3.5):
        jLog = sorted(self.journeyLogs[vehicle])+[[2879,2880,0]]
        capacity = 30
        j = 0
        t = jLog[j][0]
        SOC = 0.99
...
class MC_Simulation2:
    ...
    def dumbAndUncontrolled(self,power,capacity,filepath):
        ...
        for mc in range(self.nSim):
            ...
            dumb.append(sim.dumbCharge(power,capacity)[0*1440:2*1440])
            unctrl.append(sim.uncontrolledCharge(power,capacity)[0*1440:2*1440])
