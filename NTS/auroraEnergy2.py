...
for m in range(1,13):
    ...
    for d in range(1,8):
        day = str(d)
        run = NationalEnergyPrediction(day,month,vehicle='teslaS60D',
                                       smoothTimes=True,recordUsage=True)
        rtProfile = run.rtDemand # kW

        for i in range(len(rtProfile)):
            if i < 60*24:
                profiles[month][day][i] += rtProfile[i] # kW
            else:
                profiles[month][nextDay[day]][i-1440] += rtProfile[i]
...
for month in profiles:
    with open(fileStem+ms[month]+'RT.csv','w') as csvfile:
