...
cars = {'nissanLeafS':Vehicle(1647.7,29.97,0.0713,0.02206,0.84,24.0),
        ...
        'teslaXP100D':Vehicle(2727.3,45.71,-0.0555,0.0216,0.928,100.0),
...
for v in cars:
    results[v] = {}
    for c in ['motorway','rural','urban']:
        cycle = Drivecycle(16093,c)
        results[v][c] = cars[v].getEnergyExpenditure(cycle,0.5)/10

with open('vehicle_consumption_per_mile.csv','w') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['vehicle','rural','urban','motorway'])
    for v in results:
        writer.writerow([v,results[v]['rural'],results[v]['urban'],
                         results[v]['motorway']])
